 ...
 * @version $Id: update_product.php for pdf attachment 2019-11-10 117:49:16Z webchills $
 */
...
if (isset($_POST['edit']) && $_POST['edit'] == 'edit') {
  $action = 'new_product';
} elseif ((isset($_POST['products_model']) ? $_POST['products_model'] : '') . (isset($_POST['products_url']) ? implode('', $_POST['products_url']) : '') . (isset($_POST['products_name']) ? implode('', $_POST['products_name']) : '') . (isset($_POST['pdf_attachment_name']) ? implode('', $_POST['pdf_attachment_name']) : '') . (isset($_POST['products_merkmale']) ? implode('', $_POST['products_merkmale']) : '') . (isset($_POST['products_description']) ? implode('', $_POST['products_description']) : '') != '') {
  ...
  $pdf_db_filename = zen_limit_image_filename($_POST['products_pdf_attachment'], TABLE_PRODUCTS, 'products_pdf_attachment');
  $sql_data_array['products_pdf_attachment'] = zen_db_prepare_input($pdf_db_filename);
  $new_pdf_attachemnt = 'true';
  ...
  if ($_POST['image_delete'] == 1) {
    ...
    $new_image = 'false';
     ...
     if ($_POST['pdf_attachment_delete'] == 1) {
      $sql_data_array['products_pdf_attachment'] = '';
      $new_pdf_attachment= 'false';
    }
  ...
  for ($i = 0, $n = sizeof($languages); $i < $n; $i++) {
    ...
    $sql_data_array = array(
      ...
      'pdf_attachment_name' => zen_db_prepare_input($_POST['pdf_attachment_name'][$language_id]),
