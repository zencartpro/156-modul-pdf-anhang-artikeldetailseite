 ...
 * @version $Id: collect_info.php for pdf attachment 2020-07-24 16:40:50Z webchills $
 */
...
if (isset($_GET['pID']) && empty($_POST)) {
 $product = $db->Execute("SELECT pd.products_name, pd.products_merkmale, pd.products_description, pd.products_url, pd.pdf_attachment_name,
                                      p.products_id, p.products_quantity, p.products_model,
                                      p.products_image, p.products_pdf_attachment, p.products_price, p.products_virtual, p.products_weight,
  ...
  $pdf_attachment_name = isset($_POST['pdf_attachment_name']) ? $_POST['pdf_attachment_name'] : '';
...
// hidden fields not changeable on products page
  ...
  <div class="form-group">
   ...
   <?php
  $dir_attach_info = zen_build_subdirectories_array(DIR_FS_CATALOG_PDF_ATTACHMENTS);
  $default_attach_directory = substr( $pInfo->products_pdf_attachment, 0,strpos( $pInfo->products_pdf_attachment, '/')+1);
?>  
<div class="form-group">
      <?php echo zen_draw_separator('pixel_black.gif', '100%', '3'); ?>
  </div>  
   ...
   <div class="form-group">
   	<div class="col-sm-3 control-label">
     	 <?php echo zen_draw_label(TEXT_PDF_ATTACHMENT_NAME, 'pdf_attachment_name'); ?>    
     </div>
      <div class="col-sm-9 col-md-6">
<?php
    for ($i=0, $n=sizeof($languages); $i<$n; $i++) {
?>
      ...
      <div class="input-group">
          <span class="input-group-addon">
              <?php echo zen_image(DIR_WS_CATALOG_LANGUAGES . $languages[$i]['directory'] . '/images/' . $languages[$i]['image'], $languages[$i]['pdf_attachment_name']); ?>
          </span>
          <?php echo zen_draw_input_field('pdf_attachment_name[' . $languages[$i]['id'] . ']', htmlspecialchars(isset($pdf_attachment_name[$languages[$i]['id']]) ? stripslashes($pdf_attachment_name[$languages[$i]['id']]) : zen_get_pdf_attachment_name($pInfo->products_id, $languages[$i]['id']), ENT_COMPAT, CHARSET, TRUE), zen_set_field_length(TABLE_PRODUCTS_DESCRIPTION, 'pdf_attachment_name') . ' class="form-control"'); ?>
        </div>
         <br>
...
<div class="form-group">
     	 <?php echo zen_draw_label(TEXT_PRODUCTS_PDF_ATTACHMENT, 'products_pdf_attachment', 'class="col-sm-3 control-label"'); ?>
    <div class="col-sm-9 col-md-9 col-lg-6">
      <div class="col-md-6">
        <div class="row">
            <?php echo zen_draw_file_field('products_pdf_attachment', '', 'class="form-control"'); ?>
        </div>
        <div class="row">&nbsp;</div>
        <div class="row">
            <?php echo zen_draw_label(TEXT_PDF_ATTACHMENT_CURRENT, 'products_previous_pdf_attachment', 'class="control-label"') . '&nbsp;' . ($pInfo->products_pdf_attachment != '' ? $pInfo->products_pdf_attachment : NONE); ?>
            <?php echo zen_draw_hidden_field('products_previous_pdf_attachment', $pInfo->products_pdf_attachment); ?>
        </div>
        <div class="row">&nbsp;</div>
      </div>
      <div class="col-md-6">
        <div class="row">
          <?php echo zen_draw_label(TEXT_PRODUCTS_PDF_ATTACHMENT_DIR, 'pdf_attachment_dir', 'class="control-label"'); ?>&nbsp;<?php echo zen_draw_pull_down_menu('pdf_attachment_dir', $dir_attach_info, $default_attach_directory, 'class="form-control"'); ?>
        </div>
        <div class="row">&nbsp;</div>
        <div class="row">
            <?php echo zen_draw_label(TEXT_PDF_ATTACHMENT_DELETE, 'pdf_attachment_delete', 'class="control-label"'); ?>
          <label class="radio-inline"><?php echo zen_draw_radio_field('pdf_attachment_delete', '0', true) . TABLE_HEADING_NO; ?></label>
          <label class="radio-inline"><?php echo zen_draw_radio_field('pdf_attachment_delete', '1', false) . TABLE_HEADING_YES; ?></label>
        </div>
        <div class="row">&nbsp;</div>
        <div class="row">
            <?php echo zen_draw_label(TEXT_PDF_ATTACHMENT_OVERWRITE, 'overwrite_pdf_attachment', 'class="control-label"'); ?>
          <label class="radio-inline"><?php echo zen_draw_radio_field('overwrite_pdf_attachment', '0', false) . TABLE_HEADING_NO; ?></label>
          <label class="radio-inline"><?php echo zen_draw_radio_field('overwrite_pdf_attachment', '1', true) . TABLE_HEADING_YES; ?></label>
        </div>
        <div class="row">&nbsp;</div>
     <div class="row">
            <?php echo zen_draw_label(TEXT_PRODUCTS_PDF_ATTACHMENT_MANUAL, 'products_pdf_attachment_manual', 'class="control-label"') . zen_draw_input_field('products_pdf_attachment_manual', '', 'class="form-control"'); ?>
        </div>
      </div>
    </div>              
</div>   
