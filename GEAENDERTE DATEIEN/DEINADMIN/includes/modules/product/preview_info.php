 ...
 * @version $Id: preview_info.php for pdf attachment 2019-11-09 14:49:16Z webchills $
 */
...
if (empty($pdf_attachment_name)) $pdf_attachment_name = [];
if (empty($products_url)) $products_url = [];

if (zen_not_null($_POST)) {
  $pInfo = new objectInfo($_POST);
  $products_name = $_POST['products_name'];
  $pdf_attachment_name = $_POST['pdf_attachment_name'];
  $products_merkmale = $_POST['products_merkmale'];
  $products_description = $_POST['products_description'];
  $products_url = $_POST['products_url'];
} else {
  $product = $db->Execute("SELECT p.products_id, pd.language_id, pd.products_name, pd.pdf_attachment_name, pd.products_merkmale,
                                  pd.products_description, pd.products_url, p.products_quantity,
                                  p.products_model, p.products_image, p.products_pdf_attachment, p.products_price, p.products_virtual,
  ...
  $pdf_attachment_name = $pInfo->products_pdf_attachment;

  foreach($product as $prod) {
    $products_name[$prod['language_id']] = $prod['products_name'];
    $pdf_attachment_name[$prod['language_id']] = $prod['pdf_attachment_name'];
...
<div class="container-fluid">
    ...
    for ($i = 0, $n = sizeof($languages); $i < $n; $i++) {
      if (isset($_GET['read']) && ($_GET['read'] == 'only')) {
        ...
        $pInfo->pdf_attachment_name = zen_get_pdf_attachment_name($pInfo->products_id, $languages[$i]['id']);
        ...
        $pInfo->pdf_attachment_name = zen_db_prepare_input($pdf_attachment_name[$languages[$i]['id']]);
...
if (isset($_POST['products_pdf_attachment_manual']) && $_POST['products_pdf_attachment_manual'] != '') {
              $products_pdf_attachment_name = $_POST['pdf_attachment_dir'] . $_POST['products_pdf_attachment_manual'];
  ...
  if (isset($_GET['read']) && ($_GET['read'] == 'only')) {
    ...
    <div class="row text-right">
        ...
        for ($i = 0, $n = sizeof($languages); $i < $n; $i++) {
          ...
          echo zen_draw_hidden_field('pdf_attachment_name[' . $languages[$i]['id'] . ']', htmlspecialchars(stripslashes($pdf_attachment_name[$languages[$i]['id']]), ENT_COMPAT, CHARSET, TRUE));
        ...
        echo zen_draw_hidden_field('products_pdf_attachment', stripslashes($products_pdf_attachment_name));
